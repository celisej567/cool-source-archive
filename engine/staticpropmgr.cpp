...
static bool UTIL_ScaleStaticProp(vcollide_t* pCollide, float flScale)
{
	CPhysCollide** newCollides = (CPhysCollide**)alloca(pCollide->solidCount * sizeof(CPhysCollide*));
	for (int j = 0; j < pCollide->solidCount; j++)
	{
		newCollides[j] = pCollide->solids[j];	// FIXME: Needs to iterate over the solids
		...
		if (flScale != 1.0f)
		{
			...
			newCollides[j] = physcollision->ConvertConvexToCollide(pConvexes, nNumConvex);
			if (newCollides[j] == NULL)
				return false;

			pCollide->solids[j] = newCollides[j];
		}
	}
}
...
void CStaticPropMgr::UnserializeModels( CUtlBuffer& buf )
{
	...
	for ( int i = 0; i < count; ++i )
	{
		...
		switch ( nLumpVersion )
		{
			...
			// NEW: this will support maps from L4D up to CS:GO
...
void CStaticPropMgr::DrawStaticProps_Fast( IClientRenderable **pProps, int count, bool bShadowDepth )
{
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		if ( !pProp->m_pModel )
			continue;
		pProp->GetColorModulation(color);
		g_pStudioRender->SetColorModulation(color);
...
ConVar pipeline_static_props("pipeline_static_props", "0");
