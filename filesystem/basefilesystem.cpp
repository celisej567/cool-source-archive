...
void CBaseFileSystem::AddMapPackFile( const char *pPath, const char *pPathID, SearchPathAdd_t addType )
{
...

	{
		...
		if (header.version == 21 && header.lumps[0].fileofs == 0)
		{
			...
			lump_21_t lump21;
			V_memcpy(&lump21, &header.lumps[LUMP_PAKFILE], sizeof(lump_t));

			header.lumps[LUMP_PAKFILE].version = lump21.version;
			header.lumps[LUMP_PAKFILE].filelen = lump21.filelen;
			header.lumps[LUMP_PAKFILE].fileofs = lump21.fileofs;
		}
