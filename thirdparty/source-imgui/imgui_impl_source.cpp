...
void SizeUpBufs(IMatRenderContext* ctx, unsigned int vtxCount, unsigned int idxCount)
{
    ...
    {
        if (g_pVB) { ctx->DestroyVertexBuffer(g_pVB); g_pVB = NULL; }
        g_VertexBufferSize = vtxCount + 1000;
        g_pVB = ctx->CreateStaticVertexBuffer(VERTEX_POSITION | VERTEX_COLOR | VERTEX_TEXCOORD_SIZE(0, 2), g_VertexBufferSize, TEXTURE_GROUP_OTHER);
        if (!g_pVB)
            return;
    }
    if (!g_pIB || g_IndexBufferSize < idxCount)
    {
        if (g_pIB) { ctx->DestroyIndexBuffer(g_pIB); g_pIB = NULL; }
    ...
    for (int n = 0; n <draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                if (pcmd->GetTexID())
                {
                    ctx->SetScissorRect(pcmd->ClipRect.x - clip_off.x, pcmd->ClipRect.y - clip_off.y, pcmd->ClipRect.z - clip_off.x, pcmd->ClipRect.w - clip_off.y, true);
                    ctx->Bind(pcmd->GetTexID());
                    ctx->BindVertexBuffer(0, g_pVB, 0, pcmd->VtxOffset, cmd_list->VtxBuffer.Size , g_pVB->GetVertexFormat());
